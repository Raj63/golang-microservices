...
func dbUpdateCommand(logger *logger.Logger, db *sdksql.DB, embedFS embed.FS) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update-db",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			DBMigrateUp(logger, db, embedFS)
			DBSeedUp(logger, db, embedFS)
			logger.Infof("* The DB is migrated/Seeded succesfully.")
